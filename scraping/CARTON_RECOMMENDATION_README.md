...
- **`pallet_optimized_recommendations.json`** ⭐ **最新版**
  - パレット最適化版の推奨データ（JSON形式）
  - パレット配置情報を含む

- **`pallet_optimized_recommendations.csv`** ⭐ **最新版**
  - パレット最適化版の推奨データ（CSV形式）
  - パレットぴったり: ○で表示
  - パレット配置: 「1段4箱×9段」形式
  - 表計算ソフトで開いて確認可能
...
```bash
# 1. Googleスプレッドシートから商品データを取得（パレット情報含む）
node fetch_spreadsheet.js

# 2. パレット最適化段ボールサイズを推奨（⭐最新版・推奨）
node recommend_carton_with_pallet.js
...
- `unit_price`: 単価
- `currency`: 通貨
- **`is_pallet_fit`**: パレット1100mm×1100mmにぴったり載るか（真偽値）⭐ **新規追加**
- **`pallet_boxes_per_layer`**: パレット1段あたりの箱数（例: 4箱）⭐ **新規追加**
- **`pallet_layers`**: パレットの段数（例: 9段）⭐ **新規追加**
- **`pallet_total_boxes`**: パレット合計箱数（例: 36箱）⭐ **新規追加**
...
## パレット最適化推奨ロジック（pallet）⭐ **最新版**

### recommend_carton_with_pallet.js

`scraping/recommend_carton_with_pallet.js`

**1100mm×1100mmパレットにぴったり載る段ボールを最優先**する最新版推奨システムです。

#### 主な特徴

1. **パレット最適化**
   - スプレッドシートの「パレット」列が「ぴったり」の実績を最優先
   - cardboard_products_2026-01-14T22-44-17.csvは全てパレットぴったり段ボール
   - パレット効率を考慮した提案

2. **パレット配置情報の自動抽出**
   - 段ボール商品名から「1段4箱×9段」などの情報を自動抽出
   - パレット1段に何箱積めるかを明示
   - 合計箱数も自動計算

3. **優先順位**
   ```
   優先度1: パレット1100mm×1100mmにぴったり載る実績段ボール
   優先度2: パレット1100mm×1100mmにぴったり載る段ボール（実績から推定）
   優先度3: その他の段ボール
   ```
...
# カスタム数量
node recommend_carton_with_pallet.js 500
```
...
商品: 黒ゴマアーモンドきな粉150g×1袋セット
  ...
  【パレットぴったり実績】黒ゴマアーモンドきな粉150g×10袋セット
    段ボール: 341×226×109mm
    梱包数: 10袋（10袋×1セット）

  【1000袋を送る場合の組み合わせ】

  パターン1: 第一候補（パレットぴったり優先）のみ
    - MA140-172: 110袋入り × 10箱 = 1100袋
      644×424×288mm (宅配140サイズ)
      パレット配置: 1段4箱×6段
    合計: 10箱、1100袋（過剰: 100袋）
```

#### 生成ファイル

- `data/pallet_optimized_recommendations.json` - JSON形式の推奨データ
- `data/pallet_optimized_recommendations.csv` - CSV形式の推奨データ
  - パレットぴったり: ○で表示
  - パレット配置: 「1段4箱×9段」形式
  - 収容数、優先度、備考なども含む
...
### ケース1: 黒ゴマアーモンドきな粉150g×1袋セット 1000袋（パレット最適化版）

#### 要件
- **1100mm×1100mmパレットにぴったり載る段ボールを優先**
- 実績段ボール（341×226×109mm、10袋入り）は「パレットぴったり」実績
- パレット1段に何箱積めるかを明示
- 複数種類の段ボールを組み合わせ可能

#### 推奨される組み合わせ

**パターン1: パレットぴったり段ボール（第一候補）**
- MA140-172: 110袋入り × 10箱 = 1100袋
- 644×424×288mm (宅配140サイズ)
- **パレット配置: 1段4箱×6段** = 24箱/パレット
- 過剰: 100袋

**パターン2: パレットぴったり段ボールの組み合わせ**
- MA140-172: 110袋入り × 6箱 = 660袋（パレット配置: 1段4箱×6段）
- MAS140-075: 110袋入り × 4箱 = 440袋（パレット配置: 1段4箱×6段）
- 合計: 10箱、1100袋（過剰: 100袋）

**パターン3: 最適化（過剰最小化）**
- 自動計算で過剰を最小化
- すべて1100mm×1100mmパレットにぴったり載る段ボール
